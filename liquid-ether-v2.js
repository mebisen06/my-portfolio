class LiquidEther {
  constructor(container, options = {}) {
    ...
    this.options = {
      ...
      resolution: 0.5,
      isBounce: false,
      colors: ['#00ff41', '#d4af37', '#FF3366'], // Theme colors
      autoDemo: true,
      autoSpeed: 0.5,
      autoIntensity: 2.2,
...
`;
    ...
    class Simulation {
      ...
      getFloatType() {
        const ext = Common.renderer.extensions;
        const supportHalfFloat = ext.has('OES_texture_half_float_linear');
        const supportFloat = ext.has('OES_texture_float_linear');
        if (supportHalfFloat) return THREE.HalfFloatType;
        if (supportFloat) return THREE.FloatType;
    ...
    // Removed forced relative positioning so the CSS `position: fixed` takes effect.
